...
# AI Image Generation Skill (v27.2)

> **Skill ID**: `ai-image-generation` v27.2
> **Worker Endpoint**: `https://ai-image.lishuhang.workers.dev` (已部署 v27.2 worker)
> **Upstream**: `https://grok.17nas.com/local-api`（必须经 worker 代理，直连会被 Cloudflare 拦截）
> **Source Reference**: `worker_v27.2.js` (2790 行，已部署)
...
### v27.2 (2026-06-25) - 当前版本

- 修复 Gemini 水印 alpha 强度不匹配：实测发现不同 Gemini 版本的水印 alpha 强度不同（如 04-club.png 的实际 alpha 是 alpha map 的 1.49 倍），导致反向混合不彻底
- 新增自动增益检测（autoDetectAlphaGain）：通过对比水印区域星形像素亮度与背景亮度，自动估算实际 alpha 增益
- 新增手动增益滑块：滑块=0 为自动检测（推荐），滑块>0 时使用 0.1-3.0 的手动增益
- removeWatermarkAlpha 函数增加 gain 参数：C_original = (C_final - gain*alpha*255) / (1-gain*alpha)
- 实测：04-club.png (2752×1536) 自动检测增益 1.47，处理后星形中心亮度差从 180 降至 2.5

### v27.1 (2026-06-25)

- 修复 Gemini 水印尺寸判断逻辑：`width > 1024 && height > 1024` 改为 `width > 1024 || height > 1024`
- 问题：16:9 裁剪图（如 2752×763）因高度 ≤ 1024 误用 48px 小水印去除，导致真实 96px 水印未被处理，同时在错误位置产生"反向星星"伪影
- 修复后：任一维度 > 1024 即使用 96px+64px margin，正确覆盖真实水印区域
- 实测：用户上传的 2752×763 图片，真实水印位于 (2591,602) 大小 ~83×76，v27.1 的 96px 区域 (2592,603)-(2688,699) 完全覆盖
...
- Worker 源码：`worker_v27.2.js`（v27.2，2790 行）
- Skill 规范：`v27.2-ai-image-skill.md`（本文档）
- Skill 实现：`ai_image_skill_v27.2.py`（v27.2，约 760 行）
