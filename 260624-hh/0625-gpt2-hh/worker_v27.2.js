// Cloudflare Worker - AI生图 v27.2
// v27.2: 修复 Gemini 水印 alpha 强度不匹配 - 新增自动增益检测（从图像本身估算实际 alpha 强度）+ 手动增益滑块
...
<div class="help-panel" id="helpPanel">
  ...
  <div class="help-panel-body">
    ...
    <dl class="changelog">
      <dt>v27.2 (当前)</dt>
      <dd>修复 Gemini 水印 alpha 强度不匹配：实测发现不同 Gemini 版本的水印 alpha 强度不同（如 04-club.png 的实际 alpha 是 alpha map 的 1.49 倍），导致反向混合不彻底</dd>
      <dd>新增自动增益检测（autoDetectAlphaGain）：通过对比水印区域星形像素亮度与背景亮度，自动估算实际 alpha 增益，无需手动调整</dd>
      <dd>新增手动增益滑块：滑块=0 为自动检测（推荐），滑块>0 时使用 0.1-3.0 的手动增益</dd>
      <dd>removeWatermarkAlpha 函数增加 gain 参数：C_original = (C_final - gain*alpha*255) / (1-gain*alpha)</dd>

      <dt>v27.1</dt>
      <dd>修复 Gemini 水印尺寸判断逻辑：原代码 <code>width &gt; 1024 &amp;&amp; height &gt; 1024</code> 改为 <code>width &gt; 1024 || height &gt; 1024</code></dd>
      <dd>问题表现：16:9 裁剪图（如 2752×763）因高度 ≤ 1024 误用 48px 小水印去除，导致真实 96px 水印未被处理，同时在错误位置产生"反向星星"伪影</dd>
      <dd>修复后：任一维度 &gt; 1024 即使用 96px+64px margin，正确覆盖真实水印区域</dd>
