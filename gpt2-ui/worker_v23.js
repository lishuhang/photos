// Cloudflare Worker - AI生图 v23
// v23: 模型可用性预检(无需生成即可判断) + 改进超时错误信息(捕获上游实际错误) + 合并参考图按钮(移除大按钮,小按钮含拖放) + 其他模型失败自动换gpt-image-2重试 + hist-item样式按demo(日期左对齐,右侧SVG复制/下载按钮)
...
function defaultState(){return{accounts:[],abandonedAccounts:[],settings:{defaultPassword:'Ml@2026Proxy',rotationStrategy:'most-credits',autoCheckin:true,autoRegister:true,autoFallbackGpt2:true,theme:'system'},activeAccountIndex:-1,rotationIndex:0,lastAutoDay:''}}
      ...
      if(state.settings.autoFallbackGpt2 && model !== 'gpt-image-2' && entry.status !== 'success') {
        ...
        while(retryCount <= maxRetries) {
          try {
            ...
            if(!r.ok) { throw new Error(d.error||d.message||'创建任务失败'); }
            exhaustedAccounts.clear();
            ...
            const res=await pollTask(tid,acc.sessionToken,entry);
            if(res){entry.status='success';entry.images=res.images;entry.progress=100;entry.progressText='生成完成(GPT-Image-2回退)';updateHistory(entry.id,{status:entry.status,images:entry.images,progress:entry.progress,progressText:entry.progressText});return}
          } catch(e2) {
...
function saveSettings(){state.settings.defaultPassword=document.getElementById('defaultPassword').value.trim()||'Ml@2026Proxy';state.settings.rotationStrategy=document.getElementById('rotationStrategy').value;state.settings.autoCheckin=document.getElementById('autoCheckin').checked;state.settings.autoRegister=document.getElementById('autoRegisterChk').checked;state.settings.autoFallbackGpt2=document.getElementById('autoFallbackGpt2').checked;saveState();toast('设置已保存','success')}
function loadSettingsUI(){document.getElementById('defaultPassword').value=state.settings.defaultPassword||'Ml@2026Proxy';document.getElementById('rotationStrategy').value=state.settings.rotationStrategy||'most-credits';document.getElementById('autoCheckin').checked=state.settings.autoCheckin!==false;document.getElementById('autoRegisterChk').checked=state.settings.autoRegister!==false;document.getElementById('autoFallbackGpt2').checked=state.settings.autoFallbackGpt2!==false;applyTheme()}
