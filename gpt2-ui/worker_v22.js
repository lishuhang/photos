// Cloudflare Worker - AI生图 v22
// v22: 修复签到报错(concurrencyInfo/concurrencyDetail DOM缺失) + 额度不足自动换号 + 重试次数提升至3次 + 新增并发信息显示
...
/* Ref upload */
.ref-upload{width:100%;border:2px dashed var(--border);border-radius:var(--radius);padding:16px;text-align:center;cursor:pointer;color:var(--text-secondary);font-size:13px;margin-top:14px}
...
.ref-add{width:56px;height:56px;border:1px dashed var(--border);border-radius:var(--radius-xs);display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--text-secondary);font-size:18px;flex-shrink:0}
...
.hist-prompt-actions button{font-size:11px}
...
<div id="mainWrap">
  <div id="leftPanel">
    ...
    <div style="margin-top:14px">
      ...
      <div class="ref-upload" id="refUpload" onclick="document.getElementById('refFileInput').click()">+ 上传参考图</div>
  ...
  <div class="modal">
    ...
    <div class="modal-body">
      ...
      <div style="border-top:1px solid var(--border);padding-top:14px">
        ...
        <div class="form-group"><label class="checkbox-wrap"><input type="checkbox" id="autoCheckin" checked> 额度耗尽自动签到</label></div>
...
<div class="help-panel" id="helpPanel">
  ...
  <div class="help-panel-body">
    ...
    <ul>
      ...
      <li><strong>账号池管理</strong>：自动注册、手动添加、批量签到、智能轮换</li>
...
for(let i=0;i<state.accounts.length;i++){if(!state.accounts[i].disabled&&!state.accounts[i].sessionToken&&!excludeSet.has(state.accounts[i].username))try{await loginAccount(i)}catch(e){}}a=selectAccountWithExclude(excludeSet);if(a)return a;if(state.settings.autoCheckin){const today=new Date().toISOString().split('T')[0];for(const acc of state.accounts.filter(a=>!a.disabled&&a.lastCheckinDay!==today&&!excludeSet.has(a.username))){try{await checkinAccount(state.accounts.indexOf(acc))}catch(e){}}a=selectAccountWithExclude(excludeSet);if(a)return a}if(state.settings.autoRegister){try{await registerAccount()}catch(e){return null}a=selectAccountWithExclude(excludeSet);if(a)return a}return null}
...
async function executeTask(entry,model,prompt,size,hasRef,currentRefImages,isVideo,duration,qualityTier,resolutionName){
  ...
  while(retryCount<=maxRetries){
    try{
      ...
      if(!acc){entry.status='error';entry.error='账户余额不足，请先签到、充值或填写自己的 Bearer Token';entry.progressText='失败';updateHistory(entry.id,{status:entry.status,error:entry.error,progressText:entry.progressText});toast(entry.error,'error');return}
      ...
      const d=await r.json();if(!r.ok){const errMsg=d.error||d.message||'创建任务失败';
        ...
        if(errMsg.includes('insufficient_quota')||errMsg.includes('额度不足')){
          ...
          // 尝试签到该账号
          if(state.settings.autoCheckin){try{await checkinAccount(ai2)}catch(e){}}
...
function extractErrorMessage(raw){const msg=raw||'未知错误';if(msg.includes('临时下架')||msg.includes('暂不开放'))return'视频模型已临时下架，暂不可用';if(msg.includes('not an image model'))return'视频模型API端点错误，请更新版本';if(msg.includes('content_policy')||msg.includes('policy')||msg.includes('safety'))return'内容违反政策(OpenAI安全策略限制)';if(msg.includes('rate_limit'))return'请求频率超限，请稍后再试';if(msg.includes('insufficient_quota')||msg.includes('额度不足'))return'平台额度不足，请先签到或充值';if(msg.includes('超时')||msg.includes('timeout'))return'生成超时(300秒)，可能服务器繁忙';if(msg.includes('已注册'))return'当前IP今日已注册过账号';if(msg.includes('无效')||msg.includes('invalid'))return'请求参数无效: '+msg;if(msg.includes('failed'))return'生成失败: '+msg;return msg}
...
function buildHistoryElement(h){
  ...
  inner+='<div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap"><span class="hist-model">'+escHtml(h.model||'')+'</span>';
  if(h.account)inner+='<span class="hist-account">@'+escHtml(h.account)+'</span>';
  inner+='<span class="hist-time" style="margin-left:auto">'+timeStr+'</span></div>';
  inner+='<div class="hist-prompt">'+escHtml(h.prompt)+'</div>';
  inner+='<div class="hist-prompt-actions"><button class="btn btn-xs btn-ghost" onclick="copyHistPrompt(this)">复制</button><button class="btn btn-xs btn-ghost" onclick="addHistPromptToLib(this)">加入词库</button></div>';
